...
class Stage1(object):
    ...
    def parser(self):
        ...
        data = [['sysname','chassis','source_ip','interface','lag','port','port_type','address','isis','mpls','rsvp','slope-policy','egress-scheduler-policy','queue-policy','qos','queue-group']]
        ...
        for filename in self.arg:
            if filename != self.arg[0] and filename != self.arg[1] and filename != self.arg[2] and filename != self.arg[-1]:
                ...
                for i in interfaces:
                    print(i)
                    port = i["port"][:5] if i["port"] not in lag_ports and 'lag' not in i['port'] else lag_ports[i["port"]] if ':' not in i['port'] else lag_ports[i['port'][:i['port'].find(':')]]
                    data.append([
                        i["sysname"],
                        i["chassis"],
                        i["source_ip"],
                        i["interface"],
                        i['port'][4:i['port'].find(':')] if ':' in i['port'] and 'lag' in i['port'] else '' if i["port"] not in lag_ports else i["port"][4:],
                        port,
...
if __name__ == '__main__':
    print(sys.argv)
    if len(sys.argv) < 3:
        print("The arguments are invalid, format: [--stage-1] template [input_file] output_file")
    elif '1' in sys.argv[1] or '2' in sys.argv[1]:
        this = Stage1(sys.argv)
        object = this.parser()
        print(this.parser().message)
    elif '3' in sys.argv[1]:
        this = Stage3(sys.argv)
        object = this.parser()
    elif '5' in sys.argv[1]:
        this = Stage5(sys.argv)
        object = this.parser()
        print(object.message)
    elif '6' in sys.argv[1]:
        this = Stage6(sys.argv)
        obj = this.parser()
        print(obj.message)
